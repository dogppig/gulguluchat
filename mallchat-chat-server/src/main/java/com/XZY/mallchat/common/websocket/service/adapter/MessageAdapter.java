...
import com.XZY.mallchat.common.common.domain.vo.request.msg.TextMsgReq;
...
import com.XZY.mallchat.common.websocket.domain.enums.MessageStatusEnum;
...
//
    private static ChatMessageResp.MessageMark buildMsgMark(List<MessageMark> marks, Long receiveUid) {
        ...
        mark.setUserLike(Optional.ofNullable(receiveUid).filter(uid -> likeMarks.stream().anyMatch(a -> Objects.equals(a.getUid(), uid))).map(a -> YesorNoenum.YESOR_NOENUM.getStatus()).orElse(YesorNoenum.NOENUM.getStatus()));
        mark.setDislikeCount(dislikeMarks.size());
        mark.setUserDislike(Optional.ofNullable(receiveUid).filter(uid -> dislikeMarks.stream().anyMatch(a -> Objects.equals(a.getUid(), uid))).map(a -> YesorNoenum.YESOR_NOENUM.getStatus()).orElse(YesorNoenum.NOENUM.getStatus()));
        return mark;
    }
//
